...
export const getBalanceByMnemonic = async (mnemonic): Promise<[address: string, balance: string]> => {
...
const usePolkadot = ({
    ...
    const initHook = async (api, mnemonic) => {
        const { account } = createAccount(mnemonic);
        const {tokenDecimals, tokenSymbol} = await api.registry.getChainProperties()
